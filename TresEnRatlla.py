...
def playerMove():
    run = True
    while run:
        move = input('Selecciona una posició per\'X\' (1-9): ')
        try:
            move = int(move)
            if move > 0 and move < 10:
                if spaceIsFree(move):
                    ...
                    print('Aquest espai està ocupat!')
            else:
                print('Introduïu un número dins del rang!')
        except:
...
def main():
    print('Benvingut el Tres en Ratlla amb IA!')
    printBoard(board)
    ...
    while not (isBoardFull(board)):
        if not (isWinner(board, 'O')):
            ...
            print('Ho sentim, O\'ha guanyat aquesta vegada!')
            break
        ...
        if not (isWinner(board, 'X')):
            ...
            print('La X guanya aquest joc! Bon Treball!')
            break
...
while True:
    answer = input('Vols jugar de nou? (Y/N): ')
